...
const NewsEventsPage = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        const section = queryParams.get("section") || "announcements"; // Default to 'announcements'
        setActiveSection(section);

        const response = await axios.get(`${baseURL}/api/posts?page=news&section=${section}`);
  ...
  const sections = [
    { key: "announcements", text: "newsEvents.announcements" },
    { key: "events", text: "newsEvents.events" },
    { key: "webinars-workshops", text: "newsEvents.webinarsWorkshops" },
    { key: "press-releases", text: "newsEvents.pressReleases" },
  ];
            ...
            <div className="flex justify-center mb-8">
              <nav className="flex space-x-4">
                {sections.map((section) => (
                  <Link
                    key={section.key}
                    to={`/news-events?section=${section.key}`}
